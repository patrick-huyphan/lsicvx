...
public class BasicPhaseIPDM {
	...
	public DoubleMatrix1D findFeasibleInitialPoint() throws JOptimizerException{
	  ...
	  // Inequality constraints: fi(X)-s
		IConvexMultivariateRealFunction[] inequalities = new IConvexMultivariateRealFunction[originalProblem.getMieq()];
		for(int i=0; i<inequalities.length; i++){
			
			final IConvexMultivariateRealFunction originalFi = originalProblem.getFi()[i];
