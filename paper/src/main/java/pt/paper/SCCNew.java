...
public class SCCNew  extends Clustering{
    ...
    double[] xAvr;
    /**
     * with convex optimization, set start point and solve problem with linear or quadratic programming
...
//                }
                ...
                for(int i = 0; i< numberOfVertices; i++)
                {
                    ...
                    updateX(i, D, ni[i],xAvr); //xAvr +sumD                    
                }                      
...
//Test    
     ...
     * @return 
     */
    private double[][] updateX(int i, List<EdgeNode> D, int n, double xAvr[])
    {
...
//        Matrix.updateRow(X, Vector.plus(X[i], sumd), i); //
     ...
     * De = A - Be + Ce
     */
    private List<EdgeNode> calcD(int i, List<EdgeNode> V, List<EdgeNode> U) //(B-C)
    {
...
//        System.out.println("paper.SCC.initU() "+ret.size());
    ...
    */
    /**
     * TODO: review code, should update with optimize problem:
     * Min (lambda*w||v1-v2|| + rho/2(||x1-v1+u1||+||x2-v2+u2||))
     * @param V
     * @param U
     * @return 
     */
    private List<EdgeNode> updateV(List<EdgeNode> V, List<EdgeNode> U) //B
    {
...
//                int i=0 ;
                ...
                EdgeNode C = getEdgeNodeData(U, v.scr, v.dst);
                EdgeNode B = getEdgeNodeData(V, v.scr, v.dst);
